...
from data_utils import prepare_circle_data
from models import UpdaterNet, ClassifierNet
from model_utils import plot_model, estimate_time_left


def train(args, updater, classifier, device, optimizer, epoch):
    updater.train()
    classifier.train()
    
    optimizer.zero_grad()
    
    x, y = prepare_circle_data(args.batch_size, device)
    z = updater(x)
    output = classifier(z)
...
def main():
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        loss = train(args, updater, classifier, device, optimizer, epoch)
        ...
        if  epoch == 1 or epoch % args.log_interval == 0:
            ...
            if not args.no_plot:
                plot_model(updater, device, f"Epoch {epoch}")
